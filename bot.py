...
async def cmd_start(
    ...
    if not await check_subscription(
        ...
        await message.answer(
            ...
            "подпишитесь на все каналы и чаты!</b>\n\n"
...
async def cb_referrals(
    ...
    text = (
        ...
        f"👥 Рефералов: "
        f"<code>{refs}</code>\n\n"
        "<i>Реферал засчитывается только после "
        "того, как человек подпишется "
        "на все каналы!</i>"
    )
...
async def cb_admin_delete_refs(
    ...
    text = (
        "🗑️ <b>Удалить рефералов</b>\n"
        "━━━━━━━━━━━━━━━━━\n\n"
        "Введите:\n"
        "<code>ID Количество</code>\n\n"
        "Например:\n"
        "<code>6662093609 10</code>\n\n"
        "⚠️ Указанное количество будет "
        "вычтено из счётчика рефералов.\n"
        "⭐ Баланс пользователя "
        "не изменится."
    )
...
async def process_add_channel(
    ...
    try:
        ...
        await message.answer(
            ...
            f"информацию о чате.\n"
            f"Ошибка: {e}\n\n"
            "Убедись, что бот добавлен "
            "в канал/чат как администратор.",
            reply_markup=back_keyboard()
        )
    ...
    await message.answer(
        ...
        f"<code>{chat_id}</code>\n\n"
        "Теперь все пользователи обязаны "
        "на него подписаться.",
        reply_markup=admin_keyboard(),
        parse_mode="HTML"
    )
