...
def main_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📢 Наш канал", url="https://t.me/eclipsedlf")]
    ])
...
async def start_cmd(message: types.Message):
    ...
    if user_id not in data["users"]:
        data["users"][user_id] = {"balance": 0.0, "refs": 0}
        data["refers"][user_id] = []
...
async def show_menu(msg):
    ...
    profile_text = (
        ...
        f"━━━━━━━━━━━━━━━━━\n"
        f"<i>💜 Фиолетово-серый стиль 2026</i>"
    )
    await msg.answer(profile_text, reply_markup=main_keyboard(), parse_mode="HTML")
...
async def leaders_cmd(call: types.CallbackQuery):
    sorted_users = sorted(data["users"].items(), key=lambda x: x[1]["refs"], reverse=True)[:5]
    text = "🏆 <b>Топ рефералов</b>\n━━━━━━━━━━━━━━━━━\n"
    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
    for i, (uid, info) in enumerate(sorted_users):
        if i < 5:
            text += f"{medals[i]} <code>{uid}</code> — {info['refs']} реф.\n"
    if not sorted_users:
        text += "Пока нет рефералов 😔"
    await call.message.edit_text(text, reply_markup=back_button(), parse_mode="HTML")

async def main():
    await bot.set_my_description("💜 Бот с фиолетово-серым стилем 2026")
