...
async def show_menu(
    ...
    # =====================================================
    # НОВЫЙ ТЕКСТ ГЛАВНОГО МЕНЮ
    # =====================================================

    caption = (
        f"✨ <b>Привет, {name}!</b>\n"
        "💎 <b>Добро пожаловать в GiftsEzz Bot!</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        ...
        logger.warning(
            f"Не удалось отправить фото меню: {e}"
        )
...
async def cmd_start(message: types.Message):
    ...
    if not await check_subscription(
        ...
        await message.answer(
            "🔒 <b>Для доступа к боту подпишитесь "
            "на наши каналы!</b>\n\n"
...
# =========================================================
# НОВАЯ КОМАНДА /top — ТАБЛИЦА ЛИДЕРОВ (без кнопок)
# =========================================================

@dp.message(Command("top"))
async def cmd_top(message: types.Message):
    """Показывает топ-10 пользователей по количеству рефералов."""

    # Получаем топ-10 (или меньше, если есть) из базы
    async with db_pool.acquire() as db:
        rows = await db.fetch("""
            SELECT user_id, username, refs
            FROM users
    ...
    if not rows:
        text = (
            "🏆 <b>ТОП ЛИДЕРОВ</b>\n"
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "📭 <i>Пока никто не пригласил ни одного друга.</i>\n"
            "Стань первым — приглашай друзей!"
        )
    else:
        # Медали для первых трёх, для остальных просто цифры
        medals = ["🥇", "🥈", "🥉"] + [f"{i+1}." for i in range(3, len(rows))]
        lines = [
            "🏆 <b>ТОП ЛИДЕРОВ ПО РЕФЕРАЛАМ</b>",
            "━━━━━━━━━━━━━━━━━━━━"
        ]
        for i, row in enumerate(rows):
            uid = str(row["user_id"])
            # Если username отсутствует, показываем сокращённый ID
            username = row["username"] or f"User_{uid[:6]}"
            username = html.escape(username)
            refs = row["refs"]
            medal = medals[i] if i < len(medals) else f"{i+1}."
            lines.append(f"{medal} <b>{username}</b> — <code>{refs}</code> реф.")
        text = "\n".join(lines)
