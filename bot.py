...
async def update_leader(user_id: int, username: str = ""):
    """Обновляет запись в таблице leaders на основе данных из users."""
    async with db_pool.acquire() as db:
        row = await db.fetchrow(
            "SELECT refs FROM users WHERE user_id = $1",
            user_id
        )
        if not row:
            await db.execute(
        ...
        refs = row["refs"]
        await db.execute("""
            INSERT INTO leaders (user_id, username, refs, updated_at)
            VALUES ($1, $2, $3, NOW())
            ON CONFLICT (user_id)
            DO UPDATE SET
...
BUTTON_TEXTS = {
    ...
    "roulette": "🎰 Рулетка",
    "withdraw": "💳 Вывод",
    "leaders": "👑 Лидеры по рефералам",   # Изменён текст кнопки
...
# =========================================================
# ЛИДЕРЫ ПО РЕФЕРАЛАМ (кнопка в меню)
# =========================================================
...
# =========================================================
# КОМАНДА /top (дублирует кнопку, но работает без кнопок)
# =========================================================
...
async def refresh_leaders(message: types.Message):
    ...
    async with db_pool.acquire() as db:
        ...
        await db.execute("""
            INSERT INTO leaders (user_id, username, refs, updated_at)
            SELECT user_id, username, refs, NOW()
            FROM users
            WHERE refs > 0
        """)
