...
def main_menu_keyboard(user_id: int):
    buttons = [
        ...
        # Синяя акцентная кнопка (через web_app)
        [
            InlineKeyboardButton(
                text="🎁 Рефералы",
                web_app=WebAppInfo(url="https://t.me/GiftsEzzChat")
            )
        ],
        # Обычные тёмные кнопки
        [
            InlineKeyboardButton(
                text="🎰 Рулетка",
...
async def process_withdraw(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ Ошибка отправки заявки. Средства возвращены.",
            reply_markup=back_keyboard()
        )
        await state.clear()
        return
...
async def process_admin_give_refs(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as db:
        await db.execute(
            "UPDATE users SET refs = refs + $1 WHERE user_id = $2",
            amount, int(target_id)
        )
...
async def process_admin_delete_refs(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as db:
        await db.execute(
            "UPDATE users SET refs = $1 WHERE user_id = $2",
            new_refs, int(target_id)
        )
